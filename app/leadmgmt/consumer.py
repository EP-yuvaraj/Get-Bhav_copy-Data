...
def bConsumerMethodhistory(symbol):
    try:
        consumer = KafkaConsumer(
        'historical-bvtest',
        bootstrap_servers=['localhost:9092'],
    ...
    for message in consumer:
        ...
        df2 = pd.DataFrame(message, columns=['DATE','OPEN','HIGH','LOW','PREV. CLOSE','VOLUME','SYMBOL','isin_code','bu_id','sub_bu_id','application_id','is_active','created_by','created_date','last_modified_by','last_modified_date'])
        df2.rename(columns = {'DATE':'historical_price_date_time','OPEN' : 'day_open_price', 'HIGH' : 'day_high_price','LOW':'day_low_price' , 'PREV. CLOSE':'day_close_price' , 'VOLUME':'day_volume' , 'SYMBOL':'ticker_id'}, inplace = True)
        print(isin[0])
        print(isin[0][0])
        df2['bu_id']=1
        df2['sub_bu_id']=1
